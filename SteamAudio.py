...
class AudioStreamerGUI:
    ...
    def setup_ui(self):
        ...
        self.bitrate_combo = ttk.Combobox(main_frame, textvariable=self.bitrate_var, width=15, state='readonly')
        self.bitrate_combo['values'] = ('64k', '96k', '128k', '160k', '192k', '224k', '256k', '288k', '320k')
        self.bitrate_combo.current(4)  # Default to 192k
        self.bitrate_combo.grid(row=1, column=1, sticky=tk.W, pady=5, padx=5)
    ...
    def start_streaming(self):
        ...
        try:
            ...
            # Add SRT-specific low-latency options
            if is_srt:
                ffmpeg_cmd.extend([
                    "-pkt_size", "1316",      # Optimal packet size for SRT
                    "-latency", "50000",      # 50ms SRT latency (microseconds) - lower for audio-only
                    "-tlpktdrop", "1",        # Drop packets if too late
                    "-mode", "caller",        # SRT caller mode
                    "-flush_packets", "1"     # Ensure packets flush immediately
                ])
